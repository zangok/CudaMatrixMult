...
void __launch_bounds__(GemmPolicy::max_threads_per_block) __global__ customMatmulKernel2(int M, int N, int K,
	...
	for (int k_tile_idx = 0; k_tile_idx < K ; k_tile_idx+=GemmPolicy::TILE_DIM) {
			...
			Asub[thread_row * GemmPolicy::TILE_DIM + thread_col] = A[(c_row + thread_row) * K + (k_tile_idx * GemmPolicy::TILE_DIM + thread_col)];

			Bsub[thread_row * GemmPolicy::TILE_DIM + thread_col] = B[(k_tile_idx * GemmPolicy::TILE_DIM + thread_row) * N + (c_col + thread_col)];
